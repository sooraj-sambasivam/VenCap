...
export default function Portfolio() {
  ...
  function getBoardMeeting(companyId: string): BoardMeeting | undefined {
    return (boardMeetings || []).find(
      (m) => m.companyId === companyId && !m.attended,
    );
  }
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {portfolio.filter((c) => c.status === "active").length} active
            investments
          </p>
        </div>
      </div>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <Badge
                          className={ORIGIN_COLORS[company.origin]}
                          variant="secondary"
                        >
                          {company.origin}
                        </Badge>
                        <Badge
                          className={STATUS_COLORS[company.status]}
                          variant="secondary"
                        >
                          {company.status}
                        </Badge>
                      ...
                      <div className="flex items-center gap-2 sm:gap-4 text-xs sm:text-sm ml-auto flex-wrap">
                        <span className="text-muted-foreground hidden sm:inline">
                          ...
                          {(company.followOnInvested || 0) > 0 && (
                            <span className="text-xs text-muted-foreground ml-1">
                              (+{formatCurrency(company.followOnInvested)}{" "}
                              follow-on)
                            </span>
                          )}
                        </span>
                        <span className="text-muted-foreground hidden sm:inline">
                          Own:{" "}
                          <span className="text-foreground font-medium">
                            {formatPercent(company.ownership)}
                          </span>
                        </span>
                        ...
                        <span
                          className={`font-bold ${multipleColor(company.multiple)}`}
                        >
                          {formatMultiple(company.multiple)}
                        </span>
                        <Badge
                          className={`${FOUNDER_STATE_COLORS[company.founderState]} hidden sm:inline-flex`}
                          variant="secondary"
                        >
                          {company.founderState.replace("_", " ")}
                        </Badge>
                        <span
                          className={`font-medium ${pmfColor(company.pmfScore)}`}
                        >
                  ...
                  <div className="px-4 pb-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    ...
                    <Tabs defaultValue="actions" className="w-full">
                      <TabsList className="mb-4 w-full overflow-x-auto flex justify-start">
                        ...
                        {bo && (
                          <TabsTrigger value="buyout">Buyout Offer</TabsTrigger>
                        )}
                        {decs.length > 0 && (
                      ...
                      {fo && (
                        ...
                        >
                          <FollowOnSection
                            ...
                            opportunity={fo}
                            amount={
                              followOnAmount[company.id] ??
                              Math.round(fo.roundSize * 0.1)
                            }
                            onAmountChange={(v) =>
                              setFollowOnAmount((prev) => ({
                                ...prev,
                                [company.id]: v,
                              }))
                            }
                        ...
                        >
                          <BoardMeetingSection
                            ...
                            onResolve={(choicesByItemId) => {
                              ...
                              const itemCount =
                                Object.keys(choicesByItemId).length;
                              toast.success(
                                `Board meeting resolved — ${itemCount} agenda item${itemCount !== 1 ? "s" : ""} addressed.`,
                              );
                            }}
                          />
                        </TabsContent>
    ...
    <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              ...
              disabled={
                ...
                (company.founderState !== "distracted" &&
                  company.founderState !== "overconfident")
              }
            >
            ...
            <Button
              ...
              disabled={
                ...
                (company.founderState !== "burned_out" &&
                  company.founderState !== "defensive")
              }
            >
      ...
      <div className="flex gap-6 text-xs text-muted-foreground pt-2">
        <span>
          Relationship:{" "}
          <span className="text-foreground font-medium">
            {Math.round(company.relationship)}/100
          </span>
        </span>
        <span>
          Support:{" "}
          <span className="text-foreground font-medium">
            {Math.round(company.supportScore)}/100
          </span>
        </span>
        <span>
          Influence:{" "}
          <span className="text-foreground font-medium capitalize">
            {company.influence.replace("_", " ")}
          </span>
        </span>
      </div>
    </div>
  ...
  return (
    <div className="space-y-2 max-h-64 overflow-y-auto">
      {sorted.map((evt) => (
        <div
          key={evt.id}
          className="flex items-start gap-3 text-sm p-2 rounded-lg bg-secondary/30"
        >
          <span
  ...
  return (
    <div className="space-y-4">
      <div className="text-sm space-y-1">
        <p>
          ...
          <span className="font-medium">
            {formatCurrency(opportunity.roundSize)}
          </span>
        </p>
        <p>
          <span className="text-muted-foreground">Pre-Money:</span>{" "}
          <span className="font-medium">
            {formatCurrency(opportunity.preMoneyValuation)}
          </span>
        </p>
        <p>
          <span className="text-muted-foreground">Dilution if skip:</span>{" "}
          <span className="font-medium text-red-400">
            {formatPercent(opportunity.dilutionIfSkip)}
          </span>
        </p>
      </div>
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-muted-foreground">
          New ownership: {formatPercent(newOwnership)} (currently{" "}
          {formatPercent(company.ownership)})
        </p>
      </div>
...
function SecondarySection({
  ...
  const remainingOwnership =
    company.ownership * (1 - offer.offerPercentage / 100);
  const currentMoic =
    company.investedAmount > 0
      ? (company.currentValuation * (company.ownership / 100)) /
        company.investedAmount
      : 0;
  const offerVsCurrent =
    currentMoic > 0 ? (offer.offerMultiple / currentMoic - 1) * 100 : 0;

  const MARKET_BADGE: Record<string, { label: string; className: string }> = {
    bull: {
      label: "Bull Market — Seller-friendly",
    ...
    cooldown: {
      label: "Cooling Market — Buyer-favored",
      className: "bg-yellow-500/20 text-yellow-400",
    },
    hard: {
      label: "Down Market — Deep discounts",
  ...
  return (
    <div className="space-y-4">
      <Badge className={`${marketBadge.className} text-xs`}>
        {marketBadge.label}
      </Badge>
      ...
      <div className="text-sm space-y-1">
        ...
        <p>
          ...
          <span className="font-medium">
            {formatPercent(offer.offerPercentage)} of your stake
          </span>
        </p>
        <p>
          <span className="text-muted-foreground">At:</span>{" "}
          <span className="font-medium">
            {formatMultiple(offer.offerMultiple)} of invested
          </span>
        </p>
        ...
        <p>
          ...
          <span
            className={`font-medium ${currentMoic >= 1 ? "text-green-400" : "text-red-400"}`}
          >
        ...
        <p>
          ...
          <span
            className={`font-medium ${offerVsCurrent >= 0 ? "text-green-400" : "text-red-400"}`}
          >
            {offerVsCurrent >= 0 ? "+" : ""}
            {offerVsCurrent.toFixed(1)}%
          </span>
        </p>
        <Separator className="my-2" />
        <p>
        ...
        <p>
          ...
          <span className="font-medium">
            {formatPercent(remainingOwnership)}
          </span>
        </p>
      </div>
    ...
    <div className="space-y-4">
      <div className="p-3 rounded-md bg-orange-500/10 border border-orange-500/30">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          A buyer wants to acquire this company. Accepting triggers a founder
          acceptance check.
        </p>
      </div>
      ...
      <div className="text-sm space-y-1">
        ...
        <p>
          ...
          <span className="font-medium">
            {buyerTypeLabel[offer.buyerType] || offer.buyerType}
          </span>
        </p>
        <p>
        ...
        <p>
          <span className="text-muted-foreground">
            Your share ({formatPercent(company.ownership)}):
          </span>{" "}
          <span className="font-medium text-green-400">
            {formatCurrency(cashReturned)}
          </span>
        </p>
        <p>
          <span className="text-muted-foreground">Founder acceptance:</span>{" "}
          <span
            className={`font-medium ${offer.founderAcceptance > 0.5 ? "text-green-400" : "text-yellow-400"}`}
          >
...
function TeamSection({ company }: { company: PortfolioCompany }) {
  if (company.hiredTalent.length === 0) {
    return (
      <p className="text-sm text-muted-foreground">No hired talent yet.</p>
    );
  }
  return (
    <div className="space-y-2">
...
function CapTableSection({ company }: { company: PortfolioCompany }) {
  ...
  company.coInvestors.forEach((ci, idx) => {
    ...
    const estimatedOwnership =
      Math.round(
        (100 - company.ownership) * 0.15 * (ci.reputation / 100) * 10,
      ) / 10;
  ...
  const founderOwnership = Math.max(
    0,
    Math.round((100 - company.ownership - coInvestorTotal) * 10) / 10,
  );
  entries.splice(1, 0, {
  ...
  return (
    <div className="space-y-4">
        ...
        {entries.map((entry, idx) => (
          ...
          >
            ...
            <div className="col-span-3 text-right text-xs text-muted-foreground">
              {formatCurrency(
                Math.round(company.currentValuation * (entry.ownership / 100)),
              )}
            </div>
          </div>
        ))}
      ...
      <div className="flex justify-between text-xs text-muted-foreground pt-2 border-t border-border/50 px-2">
        <span>
          Post-money valuation: {formatCurrency(company.currentValuation)}
        </span>
        <span>
          Your position:{" "}
          {formatCurrency(
            Math.round(company.currentValuation * (company.ownership / 100)),
          )}
        </span>
      </div>
    </div>
      ...
      <div className="flex items-center gap-2 p-3 rounded-lg bg-blue-500/10 border border-blue-500/20">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            ...
            {meeting.agendaItems.length !== 1 ? "s" : ""} to review. Respond
            before the next month or relationship will suffer.
          </p>
        </div>
      </div>
