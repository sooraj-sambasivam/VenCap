...
const LP_NAMES = [
  'Horizon Endowment', 'Pacific Pension Fund', 'Great Lakes Foundation',
  'Atlas Family Office', 'Sterling Capital Partners', 'Meridian Trust',
  'Blue Harbor Advisors', 'Cedar Grove Foundation', 'Pinnacle Wealth Group',
  'Northstar Retirement System', 'Redwood Capital', 'Summit Partners LP',
  'Lighthouse Family Trust', 'Iron Gate Holdings', 'Coastal Ventures LP',
]
...
export default function Index() {
  ...
  useEffect(() => {
    if (selectedTypeConfig) {
      const mid = Math.round((selectedTypeConfig.range[0] + selectedTypeConfig.range[1]) / 2)
      setRaiseTarget(mid)
    }
  }, [selectedTypeConfig])
  ...
  function startFundraising() {
    ...
    const shuffledLPs = [...LP_NAMES].sort(() => Math.random() - 0.5).slice(0, numLPs)
  ...
  return (
    <PageShell className="min-h-screen flex flex-col items-center justify-center px-4 py-12">
      ...
      {rebirthCount > 0 && (
        <div className="mb-8 px-6 py-3 rounded-lg bg-primary/10 border border-primary/30 text-center">
          <p className="text-primary font-medium">
            ...
            <span className="text-muted-foreground">(Skill Level: {skillLevel})</span>
          </p>
        </div>
      )}
      ...
      <div className="w-full max-w-2xl">
        ...
        {step === 1 && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="text-center space-y-2">
              ...
              <p className="text-muted-foreground">Every great fund starts with a name.</p>
            </div>
        ...
        {step === 2 && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="text-center space-y-2">
              ...
              <p className="text-muted-foreground">This determines your fund size range and strategy.</p>
            </div>
                ...
                >
                  <CardContent className="p-5 space-y-3">
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg ${fundType === ft.value ? 'bg-primary/20 text-primary' : 'bg-secondary text-muted-foreground'}`}>
                        {ft.icon}
                      </div>
                      <div>
                        <h3 className="font-semibold text-lg">{ft.label}</h3>
                        <p className="text-sm text-muted-foreground">
                          {formatCurrency(ft.range[0])} – {formatCurrency(ft.range[1])}
                        </p>
                      </div>
                    </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                {(['global', 'silicon_valley', 'nyc', 'boston', 'london', 'berlin', 'singapore', 'austin', 'chicago'] as GeographicFocus[]).map((region) => {
                  const labels: Record<string, string> = {
                    global: 'Global', silicon_valley: 'Silicon Valley', nyc: 'NYC',
                    boston: 'Boston', london: 'London', berlin: 'Berlin',
                    singapore: 'Singapore', austin: 'Austin', chicago: 'Chicago',
                  }
                  return (
                    <Button
                      key={region}
                      size="sm"
                      variant={geographicFocus === region ? 'default' : 'outline'}
                      className="h-8 text-xs"
                      onClick={() => setGeographicFocus(region)}
                    >
                      {region === 'global' ? <Globe className="h-3 w-3 mr-1" /> : <MapPin className="h-3 w-3 mr-1" />}
                      {labels[region]}
                    </Button>
                  )
                })}
        ...
        {step === 3 && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="text-center space-y-2">
              ...
              <p className="text-muted-foreground">What stage of companies will you invest in?</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {STAGE_OPTIONS.map((so) => (
                ...
                >
                  <CardContent className="p-5 space-y-3">
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg ${fundStage === so.value ? 'bg-primary/20 text-primary' : 'bg-secondary text-muted-foreground'}`}>
                        {so.icon}
                      </div>
                    ...
                    <div className="flex gap-4 text-xs">
                      <span className="text-muted-foreground">
                        Check: <span className="text-foreground font-medium">{so.checkSize}</span>
                      </span>
                      <span className="text-muted-foreground">
                        Own: <span className="text-foreground font-medium">{so.ownership}</span>
                      </span>
                    </div>
                  </CardContent>
                </Card>
        ...
        {step === 4 && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="text-center space-y-2">
              ...
              <p className="text-muted-foreground">Select a scenario or play in classic Sandbox mode.</p>
            </div>
                ...
                return (
                  <Card
                    ...
                    className={`cursor-pointer transition-all duration-200 hover:border-primary/50 ${
                      isSelected ? 'border-primary bg-primary/5 shadow-lg shadow-primary/10' : 'border-border'
                    }`}
                  ...
                  >
                    <CardContent className="p-4 space-y-2">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-2">
                          <Flag className={`h-4 w-4 ${isSelected ? 'text-primary' : 'text-muted-foreground'}`} />
                          <span className="font-semibold text-sm">{scenario.name}</span>
                        </div>
                        <Badge className={`${diffColors[scenario.difficulty]} text-[10px] shrink-0`}>
                          {scenario.difficulty}
                        </Badge>
                      </div>
                      <p className="text-xs text-muted-foreground italic">{scenario.tagline}</p>
                      ...
                      {scenario.bonusScoreMultiplier > 1 && (
                        <Badge variant="outline" className="text-[10px]">×{scenario.bonusScoreMultiplier} score</Badge>
                      )}
                    </CardContent>
                  </Card>
                )
        ...
        {/* ============ STEP 5: RAISE FROM LPS ============ */}
        {step === 5 && selectedTypeConfig && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="text-center space-y-2">
              <h1 className="text-3xl font-bold">Raise From LPs</h1>
              <p className="text-muted-foreground">
                Set your target raise and launch your fundraise — or skip straight to investing.
              </p>
            </div>
              ...
              <div className="space-y-6 max-w-md mx-auto">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">Target Fund Size</span>
                  ...
                  <Slider
                    ...
                    step={selectedTypeConfig.range[0] < 50_000_000 ? 1_000_000 : 5_000_000}
                ...
                <div className="flex flex-col gap-3">
                  <Button onClick={startFundraising} className="gap-2 h-12" disabled={raiseTarget <= 0}>
                    <DollarSign className="h-4 w-4" /> Start Fundraising
                  </Button>
                  <Button variant="ghost" onClick={skipFundraising} className="gap-2 text-muted-foreground">
            ...
            {isFundraising && !fundraisingDone && (
              <div className="max-w-md mx-auto space-y-4">
                <div className="text-center">
                  <p className="text-sm text-muted-foreground mb-2">LP commitments rolling in...</p>
                  <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(
                          100,
                          (lpCommitments.reduce((s, c) => s + c.amount, 0) / raiseTarget) * 100
                        )}%`,
                      }}
                    />
                  </div>
            ...
            {fundraisingDone && (
              <div className="max-w-md mx-auto space-y-6 text-center">
                <div className="space-y-2">
                  ...
                  <p className="text-4xl font-bold text-primary">{formatCurrency(totalRaised)}</p>
                  <p className="text-sm text-muted-foreground">
                    {((totalRaised / raiseTarget) * 100).toFixed(0)}% of {formatCurrency(raiseTarget)} target
                  </p>
                </div>
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {lpCommitments.map((lp, i) => (
                    ...
                    >
                      <span>{lp.name}</span>
                      <span className="text-primary font-medium">{formatCurrency(lp.amount)}</span>
                    </div>
                  ))}
                </div>
