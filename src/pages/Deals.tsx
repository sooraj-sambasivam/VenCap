...
const SECTOR_OPTIONS = [
  'All Sectors', 'SaaS', 'Fintech', 'HealthTech', 'AI/ML', 'DevTools',
...
export default function Deals() {
  ...
  const handleRefreshDeals = useCallback(() => {
    ...
    requestAnimationFrame(() => {
      ...
      if (!store.fund) { setRefreshing(false); return }
      const newDeals: Startup[] = []
      for (let i = 0; i < 3; i++) {
        newDeals.push(
          generateStartup(store.fund.stage as FundStage, store.marketCycle, store.fund.skillLevel)
        )
      }
    ...
    <PageShell className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="ml-auto">
          <Button onClick={handleRefreshDeals} variant="secondary" className="gap-2" disabled={refreshing}>
            <RefreshCw className={`h-4 w-4 ${refreshing ? 'animate-spin' : ''}`} /> Refresh Deals
          </Button>
        </div>
      </div>
      ...
      {sortedDeals.length > 0 ? (
        ...
        <div className="text-center py-16">
          ...
          <Button onClick={handleRefreshDeals} variant="secondary" className="gap-2" disabled={refreshing}>
            <RefreshCw className={`h-4 w-4 ${refreshing ? 'animate-spin' : ''}`} /> Scout New Deals
          </Button>
        </div>
      )}
