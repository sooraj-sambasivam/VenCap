...
export default function NavBar() {
  ...
  return (
    <>
      <nav role="navigation" aria-label="Main navigation" className="sticky top-0 z-50 flex h-14 items-center border-b border-border bg-card px-4">
        {/* Logo */}
        <Link to="/" className="mr-6 text-lg font-bold tracking-tight text-foreground">
          VenCap
        </Link>
        ...
        {fund && fund.currentMonth !== undefined && (
          <>
            ...
            <span className="mr-3 text-xs text-muted-foreground sm:hidden">
              Y{getGameYear(fund.currentMonth)}, {getMonthName(fund.currentMonth).slice(0, 3)}
            </span>
          </>
        )}
            ...
            <Sheet open={open} onOpenChange={setOpen}>
              ...
              <SheetContent side="left" showCloseButton={false} className="w-64 p-0">
                <div className="flex h-14 items-center border-b border-border px-4">
                  <span className="text-lg font-bold tracking-tight">VenCap</span>
                </div>
