...
export default function InvestModal({ startup, open, onClose }: InvestModalProps) {
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-1">
            ...
            {isOwnershipOutOfRange && (
              <p className="text-xs text-red-500">
                Ownership is outside the normal range for {startup.stage.replace("_", "-")} stage deals.
              </p>
            )}
          </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {startup.coInvestors.map((ci) => (
                  <Badge key={ci.name} variant={TIER_VARIANTS[ci.tier] ?? "outline"}>
                    {ci.name}
          ...
          {startup.founderWillingness < 40 && (
            <p className="text-xs text-yellow-500">
              Founder willingness is low ({startup.founderWillingness}/100). There is a
              significant chance the founder will reject this investment.
            </p>
          )}
          ...
          <Button
            ...
            disabled={!validation.allowed || confirming}
            onClick={handleConfirm}
          >
            {confirming && <Loader2 className="h-4 w-4 animate-spin" />}
            {confirming ? 'Confirming...' : `Invest ${formatCurrency(amount)} for ${ownership.toFixed(1)}% ownership`}
          </Button>
        </div>
      </DialogContent>
