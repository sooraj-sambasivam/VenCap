...
describe("calculateBuyoutAcceptance", () => {
  ...
  it("returns probability between 0 and 1", () => {
    const result = calculateBuyoutAcceptance(
      baseStartup,
      15_000_000,
      50,
      "normal",
  ...
  it("low runway increases acceptance probability", () => {
    const lowRunway = {
      ...baseStartup,
      metrics: { ...baseStartup.metrics, runway: 3 },
    };
    const normal = calculateBuyoutAcceptance(
      baseStartup,
      15_000_000,
      50,
      "normal",
    );
    const desperate = calculateBuyoutAcceptance(
      lowRunway,
      15_000_000,
      50,
      "normal",
  ...
  it("gritty founder reduces acceptance probability", () => {
    const gritty = {
      ...baseStartup,
      founderTraits: { ...baseStartup.founderTraits, grit: 9 },
    };
    const normal = calculateBuyoutAcceptance(
      baseStartup,
      15_000_000,
  ...
  it("hard market increases probability", () => {
    const normalMarket = calculateBuyoutAcceptance(
      baseStartup,
      15_000_000,
      50,
      "normal",
    );
    const hardMarket = calculateBuyoutAcceptance(
      baseStartup,
      15_000_000,
      50,
      "hard",
...
describe("getInfluenceModifiers", () => {
  ...
  it("majority ownership has strongest modifiers", () => {
    ...
    expect(m.failMod).toBe(0.6);
    expect(m.exitMod).toBe(1.2);
  });
});
