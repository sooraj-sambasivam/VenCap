...
export const REGION_MODIFIERS: Record<
  StartupRegion,
  {
...
> = {
  silicon_valley: {
    exitMultipleMod: 1.25,
    regulatoryRiskMod: 0.9,
    tier1CoInvestorMod: 1.3,
    talentQualityMod: 1.2,
    label: "Silicon Valley",
    shortLabel: "SV",
  },
  nyc: {
    exitMultipleMod: 1.1,
    regulatoryRiskMod: 1.1,
    tier1CoInvestorMod: 1.1,
    talentQualityMod: 1.1,
    label: "New York City",
    shortLabel: "NYC",
  },
  boston: {
    exitMultipleMod: 1.05,
    regulatoryRiskMod: 1.0,
    tier1CoInvestorMod: 0.9,
    talentQualityMod: 1.15,
    label: "Boston",
    shortLabel: "BOS",
  },
  london: {
    exitMultipleMod: 1.0,
    regulatoryRiskMod: 1.3,
    tier1CoInvestorMod: 0.8,
    talentQualityMod: 1.0,
    label: "London",
    shortLabel: "LON",
  },
  berlin: {
    exitMultipleMod: 0.95,
    regulatoryRiskMod: 1.2,
    tier1CoInvestorMod: 0.7,
    talentQualityMod: 1.0,
    label: "Berlin",
    shortLabel: "BER",
  },
  singapore: {
    exitMultipleMod: 1.0,
    regulatoryRiskMod: 1.25,
    tier1CoInvestorMod: 0.75,
    talentQualityMod: 1.0,
    label: "Singapore",
    shortLabel: "SG",
  },
  austin: {
    exitMultipleMod: 0.9,
    regulatoryRiskMod: 0.85,
    tier1CoInvestorMod: 0.65,
    talentQualityMod: 0.95,
    label: "Austin",
    shortLabel: "ATX",
  },
  chicago: {
...
const ALL_REGIONS: StartupRegion[] = [
  "silicon_valley",
  "nyc",
  "boston",
  "london",
...
function generateStrategicRivalName(sector: string): string {
  const sectorKey =
    Object.keys(SECTOR_DATA).find((k) => k === sector) ?? "SaaS";
...
export function generateAcquirerName(
  type: AcquirerType,
  sector?: string,
): string {
  switch (type) {
    case "faang":
      return pickRandom(FAANG_ACQUIRERS);
    case "enterprise":
      return pickRandom(ENTERPRISE_ACQUIRERS);
    case "pe":
      return pickRandom(PE_ACQUIRERS);
    case "acquihire":
      return pickRandom(ACQUIHIRE_COMPANIES);
    case "strategic_rival":
      return generateStrategicRivalName(sector ?? "SaaS");
  }
}
...
export function getAcquirerMultipleRange(type: AcquirerType): {
  min: number;
  max: number;
} {
  switch (type) {
...
function generateUnitEconomics(stage: FundStage): UnitEconomics {
  ...
  const paybackMonths =
    monthlyGrossProfit > 0 ? Math.round(cac / monthlyGrossProfit) : 36;
...
function generateCompanyMetrics(stage: FundStage): CompanyMetrics {
  ...
  const burnRate = randomInt(
    Math.max(mrr * 1.5, 20000),
    Math.max(mrr * 4, 100000),
  );
  ...
  const runway =
    netBurn > 0
      ? Math.min(
          36,
          Math.max(3, randomInt(6, Math.round(24 * (mrr / burnRate + 0.5)))),
        )
      : randomInt(18, 36);
  const customers =
    stage === "pre_seed"
      ...
      : randomInt(
          10,
          5000 * (["seed", "series_a", "growth"].indexOf(stage) + 1),
        );
...
function generateMarketData(stage?: FundStage): MarketData {
  ...
  const tamGrowthRate = randomBetween(0.05, 0.3);
...
export function generateStartup(
  ...
  const skillBonus = Math.min(1, skillLevel * 0.1);

  const strengths = shuffleAndTake(
    sectorData.strengthPool,
    numStrengths + (skillLevel >= 5 ? 1 : 0),
  );
  const risks = shuffleAndTake(sectorData.riskPool, numRisks);
  const redFlags =
    Math.random() < skillBonus
  ...
  const founderWillingness = Math.min(
    100,
    Math.max(40, randomInt(40, 85) + marketWillingnessAdj[market]),
  );
  ...
  const valuation = Math.round(
    randomBetween(valRange.min, valRange.max) * marketMult,
  );
