...
export function calculateMarketConditions(
  ...
  const normalizedMarketFactor = Math.max(
    0.4,
    Math.min(2.0, publicMarketFactor),
  );
  ...
  const unemploymentDeviation =
    snapshot.unemploymentRate - BASELINE.unemployment;
  const employmentFactor = Math.max(
    0.8,
    Math.min(1.2, 1.0 - unemploymentDeviation * 0.03),
  );
  ...
  const ipoWindowOpen =
    sp500Ratio > 0.85 && snapshot.fedFundsRate < 6.0 && snapshot.ipoCount > 15;

  // VC competition: how competitive the deal environment is (higher = more competition for deals)
  const vcCompetitionLevel = clamp(
    vcActivity * 0.7 + normalizedMarketFactor * 0.3,
    0.5,
    1.5,
  );
...
function generateNarrative(snapshot: EconomicSnapshot): string {
  ...
  if (snapshot.fedFundsRate < 0.5) {
    parts.push(
      "Zero interest rate policy is flooding the market with cheap capital.",
    );
    ...
    parts.push(
      "Moderate interest rates are creating a balanced funding environment.",
    );
  } else if (snapshot.fedFundsRate < 5.5) {
    parts.push(
      "High interest rates are compressing valuations and tightening exit windows.",
    );
  } else {
    parts.push(
      "Very high interest rates are creating a challenging environment for startups.",
    );
  }
  ...
  if (snapshot.cpiYoY > 6) {
    parts.push(
      "Inflation is running hot, raising costs for startups and pressuring the Fed to keep rates high.",
    );
  ...
  if (snapshot.nasdaq > 15000) {
    parts.push(
      "Tech stocks are at elevated levels, supporting exit valuations.",
    );
  ...
  if (snapshot.vcFundingIndex > 200) {
    ...
    parts.push(
      "VC funding has dried up — good deals are available at lower valuations.",
    );
  }
...
export function describeChanges(
  ...
  const nasdaqChange =
    ((current.nasdaq - previous.nasdaq) / previous.nasdaq) * 100;
  ...
  if (current.gdpGrowthAnnualized < 0 && previous.gdpGrowthAnnualized >= 0) {
    ...
    current.gdpGrowthAnnualized >= 0 &&
    previous.gdpGrowthAnnualized < 0
  ) {
  ...
  if (cpiDiff > 1.0) {
    changes.push(
      `Inflation accelerating: CPI at ${current.cpiYoY.toFixed(1)}% YoY`,
    );
  } else if (cpiDiff < -1.0) {
  ...
  if (unemploymentDiff > 0.5) {
    changes.push(
      `Unemployment rising to ${current.unemploymentRate.toFixed(1)}%`,
    );
  } else if (unemploymentDiff < -0.5) {
    changes.push(
      `Unemployment falling to ${current.unemploymentRate.toFixed(1)}%`,
    );
  }
 ...
 */
export function getMarketSeverity(
  conditions: MarketConditions,
): "boom" | "healthy" | "neutral" | "stressed" | "crisis" {
  const score =
    conditions.valuationMultiplier * 0.3 +
