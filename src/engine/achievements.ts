...
> = {
  first_blood: (ctx) => ctx.portfolio.length > 0,
  ...
  first_follow_on: (ctx) => ctx.portfolio.some((c) => c.followOnInvested > 0),
  ...
  lp_whisperer: (ctx) => ctx.lpSentiment.score >= 90,
  ...
  deep_pockets: (ctx) => ctx.fund.deployed >= 50_000_000,
  ...
  triple_bagger: (ctx) => ctx.portfolio.some((c) => c.multiple >= 10),
  ...
  fund_returner: (ctx) => {
    ...
    return ctx.portfolio.some((c) => {
      if (c.status === "exited" && c.exitData) {
        return (
          c.exitData.exitValue * (c.ownership / 100) >= ctx.fund.currentSize
        );
      }
      return false;
    });
