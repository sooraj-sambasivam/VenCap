...
describe("calculateHireProbability", () => {
  it("returns value between 0.1 and 0.95", () => {
    const p = calculateHireProbability(
      makeCandidate(),
      makeCompany(),
      "normal",
    );
  ...
  it("lab company bonus increases probability", () => {
    const external = calculateHireProbability(
      makeCandidate(),
      makeCompany({ origin: "external" }),
      "normal",
    );
    const lab = calculateHireProbability(
      makeCandidate(),
      makeCompany({ origin: "lab" }),
      "normal",
    );
  ...
  it("leadership seniority reduces probability", () => {
    const senior = calculateHireProbability(
      makeCandidate({ seniority: "senior" }),
      makeCompany(),
      "normal",
    );
    const leader = calculateHireProbability(
      makeCandidate({ seniority: "leadership" }),
      makeCompany(),
      "normal",
    );
  ...
  it("bull market reduces probability", () => {
    const normal = calculateHireProbability(
      makeCandidate(),
      makeCompany(),
      "normal",
    );
    const bull = calculateHireProbability(
      makeCandidate(),
      makeCompany(),
      "bull",
    );
  ...
  it("hard market increases probability", () => {
    ...
    const hard = calculateHireProbability(
      makeCandidate(),
      makeCompany(),
      "hard",
    );
  ...
  it("high relationship increases probability", () => {
    const low = calculateHireProbability(
      makeCandidate(),
      makeCompany({ relationship: 30 }),
      "normal",
    );
    const high = calculateHireProbability(
      makeCandidate(),
      makeCompany({ relationship: 80 }),
      "normal",
    );
...
describe("applyHireEffects", () => {
  it("engineering hire returns pmfScore and growth effects", () => {
    const effects = applyHireEffects(
      makeCandidate({ role: "engineering" }),
      makeCompany(),
    );
  ...
  it("leadership seniority amplifies effects", () => {
    const senior = applyHireEffects(
      makeCandidate({ seniority: "senior", role: "executive" }),
      makeCompany(),
    );
    const leader = applyHireEffects(
      makeCandidate({ seniority: "leadership", role: "executive" }),
      makeCompany(),
    );
...
describe("processAlumni", () => {
  ...
  it("alumni are mid, senior, or leadership seniority (no juniors)", () => {
    const failed = Array.from({ length: 5 }, (_, i) =>
      makeCompany({ id: `f${i}`, status: "failed" }),
    );
    const alumni = processAlumni(failed);
    for (const a of alumni) {
