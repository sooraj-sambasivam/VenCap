...
describe('calculateLPSentiment', () => {
  ...
  it('returns correct level for score ranges', () => {
    ...
    const result = calculateLPSentiment(makeState())
    expect(['excellent', 'good', 'neutral', 'concerned', 'critical']).toContain(result.level)
  })
  ...
  it('higher multiples improve portfolio performance', () => {
    const low = calculateLPSentiment(makeState({
      portfolio: [makeCompany({ multiple: 0.5 })],
    }))
    const high = calculateLPSentiment(makeState({
  ...
  it('clamped to [0, 100] even with extreme inputs', () => {
    ...
    const company = makeCompany({
      multiple: 0.1,
      supportScore: 0,
      metrics: { mrr: 1000, growthRate: -0.10, churn: 0.20, burnRate: 500000, runway: 1, customers: 2 },
      events: Array.from({ length: 10 }, (_, i) => ({
        id: `e${i}`, type: 'cash_crunch', title: 'Bad', description: 'Bad',
        severity: 'severe', sentiment: 'negative', effects: {}, month: 20,
      })),
    })
    const result = calculateLPSentiment(makeState({
      portfolio: [company],
...
describe('generateLPPressureReport', () => {
  ...
  it('counts breakout companies', () => {
    ...
    const report = generateLPPressureReport(makeState({ portfolio: companies }))
...
describe('generateLPReport', () => {
  ...
  it('includes active company count in highlights', () => {
    ...
    expect(report.highlights.some((h: string) => h.includes('2 active'))).toBe(true)
  })
...
describe('getLPEffects', () => {
  it('neutral score (50) returns near-1.0 modifiers', () => {
    const sentiment: LPSentiment = {
      ...
      factors: { portfolioPerformance: 0, eventQuality: 0, valuationMomentum: 0, supportQuality: 0, deploymentPace: 0, labQuality: 0, incubatorOutput: 0, marketAdjustment: 0 },
      pressureReports: [],
    }
  ...
  it('high score (100) returns max positive modifiers', () => {
    const sentiment: LPSentiment = {
      score: 100, level: 'excellent',
      factors: { portfolioPerformance: 20, eventQuality: 15, valuationMomentum: 10, supportQuality: 10, deploymentPace: 10, labQuality: 10, incubatorOutput: 10, marketAdjustment: 15 },
      pressureReports: [],
    }
    const effects = getLPEffects(sentiment)
...
describe('calculateLPActionEffect', () => {
  ...
  it('coinvest_opportunity: blocked with empty portfolio', () => {
    const result = calculateLPActionEffect('coinvest_opportunity', 50, fund, [])
    expect(result.canPerform).toBe(false)
  })

  it('coinvest_opportunity: blocked when no company has multiple > 1.5', () => {
    const portfolio = [makeCompany({ multiple: 1.2 })] as ReturnType<typeof makeCompany>[]
    const result = calculateLPActionEffect('coinvest_opportunity', 50, fund, portfolio as never)
    expect(result.canPerform).toBe(false)
  })

  it('coinvest_opportunity: succeeds with active company multiple > 1.5', () => {
    const portfolio = [makeCompany({ multiple: 2.0, status: 'active' })] as ReturnType<typeof makeCompany>[]
    const result = calculateLPActionEffect('coinvest_opportunity', 50, fund, portfolio as never)
    expect(result.canPerform).toBe(true)
  ...
  it('early_distribution: blocked before year 3 (month < 24)', () => {
    ...
    const result = calculateLPActionEffect('early_distribution', 50, earlyFund, [], { amount: 1_000_000 })
    expect(result.canPerform).toBe(false)
  ...
  it('early_distribution: blocked without amount specified', () => {
    ...
    const result = calculateLPActionEffect('early_distribution', 50, lateFund, [])
    expect(result.canPerform).toBe(false)
  ...
  it('early_distribution: blocked when amount exceeds cash', () => {
    ...
    const result = calculateLPActionEffect('early_distribution', 50, lateFund, [], { amount: 5_000_000 })
    expect(result.canPerform).toBe(false)
    expect(result.reason).toContain('cash')
  })
  ...
  it('early_distribution: succeeds in year 3+, scales sentiment with amount', () => {
    const lateFund = { ...fund, currentMonth: 36 }
    const small = calculateLPActionEffect('early_distribution', 50, lateFund, [], { amount: 100_000 })
    const large = calculateLPActionEffect('early_distribution', 50, lateFund, [], { amount: 10_000_000 })
  ...
  it('returns canPerform: false when action is on cooldown', () => {
    const fundOnCooldown = {
      ...fund,
      lpActionCooldowns: [{ actionType: 'quarterly_update' as const, availableFromMonth: 99 }],
    }
    const result = calculateLPActionEffect('quarterly_update', 50, fundOnCooldown, [])
    expect(result.canPerform).toBe(false)
  ...
  it('ignores expired cooldowns', () => {
    const fundExpiredCooldown = {
      ...fund,
      currentMonth: 30,
      lpActionCooldowns: [{ actionType: 'quarterly_update' as const, availableFromMonth: 10 }],
    }
    const result = calculateLPActionEffect('quarterly_update', 50, fundExpiredCooldown, [])
    expect(result.canPerform).toBe(true)
  })
})
