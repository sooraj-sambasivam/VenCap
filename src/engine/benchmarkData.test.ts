...
describe("getBenchmarkForFund", () => {
  it("returns a valid BenchmarkComparison structure", () => {
    ...
    expect(result.benchmarkTvpi.topQ).toBeGreaterThan(
      result.benchmarkTvpi.median,
    );
    expect(result.benchmarkTvpi.median).toBeGreaterThan(
      result.benchmarkTvpi.bottomQ,
    );
  ...
  it("classifies player between median and top as second_quartile", () => {
    ...
    const midUpperTvpi =
      (ref.benchmarkTvpi.median + ref.benchmarkTvpi.topQ) / 2;
    const result = getBenchmarkForFund(
      "national",
      "seed",
      2015,
      midUpperTvpi,
  ...
  it("classifies player between bottom and median as third_quartile", () => {
    const ref = getBenchmarkForFund("national", "seed", 2015, 1.5, 12);
    const midLowerTvpi =
      (ref.benchmarkTvpi.bottomQ + ref.benchmarkTvpi.median) / 2;
    const result = getBenchmarkForFund(
      "national",
      "seed",
      2015,
  ...
  it("maps different fund types to appropriate benchmark categories", () => {
    ...
    const growthResult = getBenchmarkForFund(
      "national",
      "growth",
      2015,
      2.0,
    ...
    expect([
      ...
      "third_quartile",
      "bottom_quartile",
    ]).toContain(seedResult.playerTvpiPercentile);
    expect([
      "top_quartile",
      "second_quartile",
      "third_quartile",
      "bottom_quartile",
    ]).toContain(growthResult.playerTvpiPercentile);
  });
  ...
  it("benchmark IRR values are ordered correctly", () => {
    const result = getBenchmarkForFund("national", "seed", 2015, 1.5, 12);
    expect(result.benchmarkIrr.topQ).toBeGreaterThan(
      result.benchmarkIrr.median,
    );
    expect(result.benchmarkIrr.median).toBeGreaterThan(
      result.benchmarkIrr.bottomQ,
    );
  });
});
