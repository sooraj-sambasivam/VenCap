...
describe("generateMonthlyEvents", () => {
  ...
  it("stressed companies get more negative events over time", () => {
    ...
    let healthyNeg = 0,
      stressedNeg = 0;
    for (let i = 0; i < 200; i++) {
      healthyNeg += generateMonthlyEvents(healthy, "normal").filter(
        (e) => e.sentiment === "negative",
      ).length;
      stressedNeg += generateMonthlyEvents(stressed, "hard").filter(
        (e) => e.sentiment === "negative",
      ).length;
    }
...
describe("applyEventModifiers", () => {
  it("does not modify positive events", () => {
    const positiveEvent = makeEvent({
      sentiment: "positive",
      effects: { growthMod: 0.1 },
    });
    const modified = applyEventModifiers(positiveEvent, makeCompany());
    expect(modified.effects.growthMod).toBe(0.1);
  });
  ...
  it("incubator origin also reduces negative effects but less than lab", () => {
    ...
    expect(labResult.effects.mrrMod!).toBeGreaterThan(
      incubResult.effects.mrrMod!,
    );
  });
  ...
  it("high support score reduces negative effects", () => {
    ...
    expect(highResult.effects.mrrMod!).toBeGreaterThan(
      lowResult.effects.mrrMod!,
    );
  });
  ...
  it("board_seat influence reduces negative effects", () => {
    ...
    expect(boardResult.effects.failChanceMod!).toBeLessThan(
      obsResult.effects.failChanceMod!,
    );
  });
  ...
  it("multiple protections stack", () => {
    const event = makeEvent();
    const unprotected = makeCompany({
      origin: "external",
      supportScore: 20,
      influence: "observer",
      relationship: 30,
    });
    const maxProtected = makeCompany({
      origin: "lab",
      supportScore: 80,
    ...
    expect(protResult.effects.mrrMod!).toBeGreaterThan(
      unprotResult.effects.mrrMod!,
    );
    expect(protResult.effects.failChanceMod!).toBeLessThan(
      unprotResult.effects.failChanceMod!,
    );
  });
});
...
describe("legal event sector filtering", () => {
  function getEventTypes(
    ...
    for (let i = 0; i < iterations; i++) {
      generateMonthlyEvents(company, "normal").forEach((e) =>
        types.add(e.type),
      );
    }
    return types;
  ...
  it("sec_investigation only fires for Fintech sector", () => {
    ...
    for (let i = 0; i < 5000; i++) {
      ...
      if (events.some((e) => e.type === "sec_investigation")) {
        seen = true;
        break;
      }
    }
  ...
  it("gdpr_violation fires for SaaS, Fintech, Consumer but not DeepTech", () => {
    ...
    for (const sector of ["SaaS", "Fintech", "Consumer"]) {
      ...
      for (let i = 0; i < 5000; i++) {
        ...
        if (events.some((e) => e.type === "gdpr_violation")) {
          seen = true;
          break;
        }
      }
  ...
  it("patent_dispute only fires for DeepTech and Biotech", () => {
    ...
    for (const sector of ["DeepTech", "Biotech"]) {
      ...
      for (let i = 0; i < 5000; i++) {
        const events = generateMonthlyEvents(makeCompany({ sector }), "normal");
        if (events.some((e) => e.type === "patent_dispute")) {
          seen = true;
          break;
        }
      }
  ...
  it("antitrust_scrutiny only fires for Marketplace/SaaS with MRR > 500K", () => {
    ...
    for (let i = 0; i < 5000; i++) {
      ...
      if (events.some((e) => e.type === "antitrust_scrutiny")) {
        seen = true;
        break;
      }
    }
