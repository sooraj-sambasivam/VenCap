...
import { generateStartup, generateAcquirerName, getAcquirerMultipleRange } from './mockData';
import { generateMonthlyEvents, applyEventModifiers } from './dynamicEvents';
import { calculateLPSentiment, generateLPPressureReport, generateLPReport, getLPEffects, calculateLPActionEffect } from './lpSentiment';
...
function generateMarketNews(market: MarketCycle, month: number): NewsItem {
  const templates: Record<MarketCycle, { headlines: string[]; summaries: string[] }> = {
    bull: {
      headlines: [
  ...
  return {
    ...
    type: 'market_trend',
    sentiment: market === 'bull' ? 'positive' : market === 'hard' ? 'negative' : 'neutral',
    month,
  };
}
...
const SECTORS = ['SaaS', 'Fintech', 'HealthTech', 'AI/ML', 'DevTools', 'Marketplace', 'Consumer', 'CleanTech', 'EdTech', 'Cybersecurity', 'DeepTech', 'Biotech', 'SpaceTech', 'AgTech', 'PropTech'];
const FAKE_COMPANIES = ['NovaTech', 'Lumina AI', 'VertexHQ', 'Cloudform', 'Datawise', 'Streamline', 'Greenpath', 'Finova', 'MediSync', 'BuildLayer', 'AstraCore', 'NexGen', 'Prism Labs', 'Scalepoint', 'Vaultix'];
const SERIES = ['Seed', 'A', 'B', 'C', 'D'];

function generateEcosystemNews(month: number, playerSectors: string[]): NewsItem[] {
  ...
  if (Math.random() < 0.50) {
    ...
    const amount = series === 'Seed' ? randomInt(2, 8) : series === 'A' ? randomInt(10, 30) : randomInt(30, 200);
    // Bias toward player's sectors
    const sector = playerSectors.length > 0 && Math.random() < 0.4
  ...
  if (Math.random() < 0.30) {
    const sector = playerSectors.length > 0 && Math.random() < 0.5
      ? pickRandom(playerSectors)
      : pickRandom(SECTORS);
    const trends = [
      { headline: `${sector} Seeing Increased Investor Interest`, summary: `Multiple funds have announced dedicated ${sector} practices. Deal competition in the space is heating up as more capital flows in.`, sentiment: 'positive' as const },
      { headline: `${sector} Consolidation Wave Begins`, summary: `Several ${sector} companies have merged or been acquired in recent months, signaling market maturity and potential opportunities for remaining players.`, sentiment: 'neutral' as const },
      { headline: `${sector} Headcount Growth Surges`, summary: `${sector} companies are hiring aggressively, with engineering roles up 40% quarter-over-quarter. Talent competition is intensifying across the sector.`, sentiment: 'positive' as const },
    ];
  ...
  if (Math.random() < 0.15) {
    ...
    const regs = [
      { headline: `New Regulations Proposed for ${sector}`, summary: `Government officials are proposing new compliance requirements for ${sector} companies. The impact on startups remains unclear, but larger players may benefit from increased barriers to entry.`, sentiment: 'negative' as const },
      { headline: `${sector} Gets Regulatory Clarity`, summary: `New regulatory guidelines have been issued for ${sector}, providing clearer rules of engagement. Industry groups are calling this a positive development for innovation.`, sentiment: 'positive' as const },
    ];
...
function generateDecision(company: PortfolioCompany, month: number): PendingDecision {
  type DecisionTemplate = { title: string; description: string; options: { label: string; effects: Record<string, number> }[] };
  const decisions: DecisionTemplate[] = [
    {
      ...
      options: [
        { label: 'Support the pivot', effects: { growthRate: 0.05, churn: 0.03, pmfScore: -10 } },
        { label: 'Stay the course', effects: { growthRate: -0.02, relationship: -5 } },
        { label: 'Propose a middle ground', effects: { growthRate: 0.02, relationship: 5, pmfScore: -3 } },
      ],
    },
    {
      ...
      options: [
        { label: 'Approve full plan', effects: { growthRate: 0.08, burnRate: 0.50, runway: -6 } },
        { label: 'Approve half', effects: { growthRate: 0.04, burnRate: 0.25, runway: -3 } },
        { label: 'Deny — focus on efficiency', effects: { relationship: -8, burnRate: -0.10 } },
      ],
    },
    {
      ...
      options: [
        { label: 'Go for it', effects: { growthRate: 0.06, burnRate: 0.30, pmfScore: -8 } },
    ...
    },
    {
      ...
      options: [
        { label: 'Increase prices 30%', effects: { mrr: 0.15, churn: 0.02, growthRate: -0.03 } },
        { label: 'Modest 10% increase', effects: { mrr: 0.05, churn: 0.005 } },
        { label: 'Keep prices, add premium tier', effects: { mrr: 0.08, growthRate: 0.02 } },
      ],
    },
    {
      ...
      options: [
        { label: 'Acquire them', effects: { customers: 0.30, burnRate: 0.20, pmfScore: -5 } },
        { label: 'Pass', effects: {} },
      ],
    },
  ];
...
function generateFailureReason(company: PortfolioCompany): string {
  ...
  if (company.metrics.churn > 0.10) return reasons[2];
...
const BOARD_AGENDA_TEMPLATES: Omit<BoardMeetingAgendaItem, 'id' | 'resolved' | 'chosenOptionIndex'>[] = [
  {
    type: 'budget_approval',
    title: 'Budget Approval',
    description: 'Review and approve the quarterly budget. The team needs clarity on resource allocation.',
    options: [
      { label: 'Approve aggressive budget', description: 'Full headcount + tools. High burn, high growth.', effects: { burnRate: 0.20, growthRate: 0.08 } },
      { label: 'Approve conservative budget', description: 'Controlled spending. Extends runway.', effects: { burnRate: -0.10, growthRate: -0.03 } },
      { label: 'Send back for revision', description: 'Request cleaner assumptions.', effects: { relationship: -5 } },
    ],
  },
  {
    type: 'pivot_evaluation',
    title: 'Pivot Evaluation',
    description: 'The team has identified a potentially more attractive market. Evaluate whether to pivot.',
    options: [
      { label: 'Approve the pivot', description: 'New direction. Risky but could unlock growth.', effects: { pmfScore: -15, growthRate: 0.10 } },
      { label: 'Stay the course', description: 'Double down on current strategy.', effects: { pmfScore: 5, growthRate: -0.02 } },
      { label: 'Run a 60-day experiment', description: 'Test before committing.', effects: { pmfScore: -3, relationship: 5 } },
    ],
  },
  {
    type: 'hiring_plan',
    title: 'Hiring Plan Review',
    description: 'Management wants to double engineering headcount. Weigh growth vs. burn.',
    options: [
      { label: 'Approve full plan', description: '10+ new hires. Significant burn increase.', effects: { burnRate: 0.35, growthRate: 0.07 } },
      { label: 'Approve half the plan', description: '5 hires. Balanced approach.', effects: { burnRate: 0.15, growthRate: 0.04 } },
      { label: 'Freeze hiring', description: 'Preserve cash.', effects: { burnRate: -0.05, relationship: -8 } },
    ],
  },
  {
    type: 'pricing_strategy',
    title: 'Pricing Strategy',
    description: 'The team wants to restructure pricing. Could improve margins or drive growth.',
    options: [
      { label: 'Premium tier model', description: 'Higher prices for enterprise. Better margins.', effects: { mrr: 0.20, churn: 0.02 } },
      { label: 'Volume pricing', description: 'Lower price, higher volume target.', effects: { mrr: -0.05, growthRate: 0.08 } },
      { label: 'Keep existing pricing', description: 'Avoid disruption.', effects: { relationship: 3 } },
    ],
  },
  {
    type: 'expansion_plan',
    title: 'Market Expansion',
    description: 'The company wants to enter a new geographic or vertical market.',
    options: [
      { label: 'Approve expansion', description: 'New market entry. Dilutes focus.', effects: { growthRate: 0.06, burnRate: 0.25, pmfScore: -8 } },
      { label: 'Defer 6 months', description: 'Wait until core market is stronger.', effects: { relationship: -3 } },
    ],
  },
  {
    type: 'ma_review',
    title: 'Acquisition Opportunity Review',
    description: 'A competitor is struggling and might be acquired. Board approval needed.',
    options: [
      { label: 'Approve the acquisition', description: 'Buy the competitor. Customers + debt.', effects: { mrr: 0.25, burnRate: 0.30, pmfScore: -5 } },
      { label: 'Pass on acquisition', description: 'Stay focused on organic growth.', effects: {} },
    ],
  },
  {
    ...
    options: [
      { label: 'Approve as submitted', description: 'Full scope. Ambitious timeline.', effects: { pmfScore: 5, growthRate: 0.04 } },
      { label: 'Narrow scope', description: 'Focus on must-haves only.', effects: { pmfScore: 8, burnRate: -0.10 } },
      { label: 'Request customer validation first', description: 'De-risk before committing engineering.', effects: { pmfScore: 3, relationship: 5 } },
    ],
  },
];

function generateBoardMeeting(company: PortfolioCompany, currentMonth: number): BoardMeeting {
  const numItems = randomInt(1, 3);
  const shuffled = [...BOARD_AGENDA_TEMPLATES].sort(() => Math.random() - 0.5).slice(0, numItems);
  ...
  persist(
    (set, get) => ({
      ...
      initFund: (config) => {
        ...
        const currentSize = mergedConfig.currentSize || mergedConfig.targetSize || 100_000_000;
        ...
        const fund: Fund = {
          ...
          carryRate: mergedConfig.carryRate ?? 0.20,
        ...
        const startingMarketCycle: MarketCycle = scenario?.startingMarketCycle ?? 'normal';
        const startingLPScore = scenario?.startingLPSentiment ?? 50;

        const lpSentimentInit: LPSentiment = {
          ...initialLPSentiment,
          score: startingLPScore,
          level: startingLPScore >= 80 ? 'excellent' : startingLPScore >= 60 ? 'good' : startingLPScore >= 40 ? 'neutral' : startingLPScore >= 20 ? 'concerned' : 'critical',
        };

        // Generate initial deal pipeline and talent pool
        const pipeline: Startup[] = [];
        for (let i = 0; i < 3; i++) {
          pipeline.push(generateStartup(fund.stage, startingMarketCycle, fund.skillLevel, fund.geographicFocus));
        }
        ...
        const seedPortfolio: PortfolioCompany[] =
          (scenario && (scenarioId === 'zombie_fund' || scenarioId === 'crisis_manager'))
      ...
      advanceTime: () => {
        ...
        const newFundraisingEvents: FundraisingEvent[] = [...state.fundraisingEvents];
        const newFollowOns: FollowOnOpportunity[] = [...state.followOnOpportunities];
        ...
        if (fund.carryRate === undefined) fund.carryRate = 0.20;
        ...
        if (fund.gpCommit === undefined) fund.gpCommit = Math.round(fund.currentSize * 0.01);

        const monthlyFee = Math.round((fund.currentSize * fund.managementFeeRate) / 12);
        ...
        const cycleInterval = Math.max(12, Math.round(18 / difficulty.marketCycleSpeed));
        ...
        if (monthsSinceStart > 0 && monthsSinceStart % cycleInterval === 0) {
          ...
          if (newCycle !== marketCycle) {
            ...
            newNews.push({
              ...
              type: 'cycle_change',
              sentiment: marketCycle === 'bull' ? 'positive' : marketCycle === 'hard' ? 'negative' : 'neutral',
        ...
        for (let i = 0; i < portfolio.length; i++) {
          ...
          // Source modifiers
          if (company.origin === 'lab') { failMod *= 0.80; exitMod *= 1.15; }
          if (company.origin === 'incubator') { failMod *= 0.85; exitMod *= 1.10; }

          // Support modifiers
          if (company.supportScore > 30) { failMod *= 0.60; growthMod *= 1.15; }

          // Relationship modifiers
          if (company.relationship < 30) { failMod *= 1.50; }
          else if (company.relationship > 70) { failMod *= 0.75; }
          ...
          const founderEffects: Record<FounderState, { growthMod: number; failMod: number }> = {
            focused:       { growthMod: 1.10, failMod: 1.00 },
            distracted:    { growthMod: 0.95, failMod: 1.05 },
            burned_out:    { growthMod: 0.85, failMod: 1.25 },
            overconfident: { growthMod: 1.05, failMod: 1.10 },
            defensive:     { growthMod: 0.90, failMod: 1.10 },
            coachable:     { growthMod: 1.05, failMod: 0.90 },
          };
          ...
          // PMF effects
          if (company.pmfScore > 70) { growthMod *= 2.0; failMod *= 0.80; }
          else if (company.pmfScore < 40) { growthMod *= 0.5; failMod *= 1.20; }

          // ---- 2b: Generate Dynamic Events ----
          const events = generateMonthlyEvents(company, marketCycle, company.region);
          const appliedEvents: DynamicEvent[] = [];

          for (const event of events) {
            const modified = applyEventModifiers({ ...event, month: fund.currentMonth }, company);
            appliedEvents.push(modified);

            // Apply event effects to modifiers
            if (modified.effects.failChanceMod) failMod *= (1 + modified.effects.failChanceMod);
            if (modified.effects.exitChanceMod) exitMod *= (1 + modified.effects.exitChanceMod);
            if (modified.effects.growthMod) growthMod *= (1 + modified.effects.growthMod);
            if (modified.effects.relationshipMod) {
              company.relationship = clamp(company.relationship + modified.effects.relationshipMod, 0, 100);
            }
            if (modified.effects.pmfMod) {
              company.pmfScore = clamp(company.pmfScore + modified.effects.pmfMod, 0, 100);
            }
            if (modified.effects.mrrMod) {
              company.metrics = {
                ...company.metrics,
                mrr: Math.max(0, company.metrics.mrr * (1 + modified.effects.mrrMod)),
              };
            }
          }
          ...
          if (Math.random() < exitChance) {
            ...
            const acquirerType = company.pmfScore > 70 && company.metrics.mrr > 200000
              ? weightedRandom(['faang', 'strategic_rival', 'enterprise', 'pe'] as const, [30, 30, 25, 15])
              : weightedRandom(['enterprise', 'acquihire', 'pe', 'strategic_rival'] as const, [30, 30, 25, 15]);
            ...
            const exitBase = company.investedAmount > 0 ? company.investedAmount : company.currentValuation;
          ...
          const marketCycleMod: Record<MarketCycle, number> = {
            bull: 1.15, normal: 1.0, cooldown: 0.85, hard: 0.7,
          };
          const finalGrowth = baseGrowth * marketCycleMod[marketCycle] * growthMod;

          company.metrics = {
            ...company.metrics,
            mrr: Math.max(0, Math.round(company.metrics.mrr * finalGrowth)),
            customers: Math.max(0, Math.round(company.metrics.customers * (1 + (finalGrowth - 1) * 0.5))),
            growthRate: clamp(company.metrics.growthRate * (0.95 + Math.random() * 0.1), 0, 1),
          };

          // Update valuation based on MRR growth
          company.currentValuation = Math.round(company.currentValuation * finalGrowth);
          company.multiple = company.investedAmount > 0
            ? Math.round((company.currentValuation / company.investedAmount) * 100) / 100
            : 0;
          ...
          if (
            ...
            newFollowOns.push({
              companyId: company.id,
              roundSize: Math.round(company.currentValuation * randomBetween(0.15, 0.30)),
          ...
          if (
            (company.influence === 'board_seat' || company.influence === 'majority') &&
            Math.random() < 0.25 &&
          ...
          const hasPendingMeeting = newBoardMeetings.some(m => m.companyId === company.id && !m.attended);
          if (!hasPendingMeeting && monthsActive2 > 0) {
            if (company.influence === 'board_seat' && monthsActive2 % 6 === 0) {
              newBoardMeetings.push(generateBoardMeeting(company, fund.currentMonth));
            } else if (company.influence === 'majority' && monthsActive2 % 12 === 0) {
              newBoardMeetings.push(generateBoardMeeting(company, fund.currentMonth));
            }
          }
          ...
          if (
            ...
            newSecondaries.push({
              id: uuid(),
              companyId: company.id,
              buyerName: pickRandom(['Lexington Partners', 'Blackstone Secondaries', 'Coller Capital', 'HarbourVest', 'Adams Street Partners']),
          ...
          if (
            ...
            const buyerNames: Record<string, string[]> = {
              pe: ['Summit Equity', 'Horizon Capital Partners', 'Apex Growth Fund', 'Meridian PE'],
              strategic: ['TechCorp Global', 'Innovate Holdings', 'Atlas Enterprises', 'Nexus Group'],
              rival_fund: ['Benchmark Capital', 'Lighthouse Ventures', 'Pinnacle Partners', 'Vista Growth'],
            };
            ...
            const acceptance = calculateBuyoutAcceptance(company, offerPrice, company.relationship, marketCycle);
          ...
          let decayRate = 1;
          if (company.origin === 'lab' || company.origin === 'incubator') decayRate = 0.5;
          if (company.ownership > 50) decayRate = 0.5;
          if (company.influence === 'board_seat' || company.influence === 'majority') decayRate = 0;
          ...
          if (Math.random() < 0.20) {
            ...
            if (growthTrend > 0.10 && rel > 60) {
              newState = weightedRandom(['focused', 'coachable'] as const, [60, 40]);
            } else if (growthTrend < 0.03 && rel < 40) {
              newState = weightedRandom(['defensive', 'burned_out'] as const, [50, 50]);
        ...
        const cleanedBoardMeetings: BoardMeeting[] = newBoardMeetings.filter(m => {
          if (!m.attended && m.scheduledMonth < fund.currentMonth) {
            ...
            if (compIdx >= 0) {
              portfolio[compIdx] = {
                ...portfolio[compIdx],
                relationship: Math.max(0, portfolio[compIdx].relationship - 5),
                events: [...portfolio[compIdx].events, {
                  id: uuid(),
        ...
        if (fund.currentMonth > 0 && fund.currentMonth % 12 === 0) {
          ...
          sentiment.pressureReports = [...state.lpSentiment.pressureReports, pressureReport];
          ...
          if (fund.deployed / fund.currentSize > 0.70 && sentiment.score > 55 && year >= 3) {
            const callAmount = Math.round(fund.currentSize * randomBetween(0.05, 0.15));
          ...
          const totalExitValue = portfolio
            ...
            .reduce((sum, c) => sum + (c.exitData?.exitValue || 0) * (c.ownership / 100), 0);
          if (totalExitValue > fund.currentSize * 0.20 && year >= 4) {
            const distributionAmount = Math.round(totalExitValue * randomBetween(0.10, 0.30));
            if (distributionAmount > 0 && fund.cashAvailable >= distributionAmount) {
              newFundraisingEvents.push({
                id: uuid(),
                type: 'distribution',
          ...
          if (year >= 7 && sentiment.score > 65 && fund.tvpiEstimate > 1.5) {
            const fund2Size = Math.round(fund.currentSize * randomBetween(1.2, 2.0));
            newFundraisingEvents.push({
              id: uuid(),
              type: 'fund_ii_offer',
        ...
        for (let i = 0; i < 3; i++) {
          ...
          if (lpEffects.dealflowMod > 1.05) {
            ...
            deal.founderTraits.clarity = Math.min(10, deal.founderTraits.clarity + 1);
          }
        ...
        const totalValue = portfolio.reduce((sum, c) => {
          if (c.status === 'exited') return sum + (c.exitData?.exitValue || 0) * (c.ownership / 100);
          if (c.status === 'active') return sum + c.currentValuation * (c.ownership / 100);
          return sum;
        }, fund.cashAvailable + fund.totalFeesCharged); // Add fees back for gross calculation
        fund.tvpiEstimate = fund.currentSize > 0
        ...
        if (years > 0 && fund.tvpiEstimate > 0) {
          fund.irrEstimate = Math.round((Math.pow(fund.tvpiEstimate, 1 / years) - 1) * 100 * 100) / 100;
        }
        ...
        if (gamePhase === 'playing' && state.activeScenario && state.activeScenario.id !== 'sandbox') {
          const sc = state.activeScenario;
          const exitsCount = portfolio.filter(c => c.status === 'exited').length;

          // Check each win condition
          const allMet = sc.winConditions.length > 0 && sc.winConditions.every(cond => {
          ...
          if (sc.id === 'lp_rescue' && fund.currentMonth >= 36 && state.lpSentiment.score < 60) {
            gamePhase = 'ended';
            scenarioWon = false;
          }
        }
        ...
        if (gamePhase === 'ended' && scenarioWon === null && state.activeScenario && state.activeScenario.id !== 'sandbox') {
          const sc = state.activeScenario;
          const exitsCount = portfolio.filter(c => c.status === 'exited').length;
          scenarioWon = sc.winConditions.length > 0 && sc.winConditions.every(cond => {
            if (cond.type === 'tvpi') return fund.tvpiEstimate >= cond.threshold;
            if (cond.type === 'lp_sentiment') return state.lpSentiment.score >= cond.threshold;
            if (cond.type === 'exits') return exitsCount >= cond.threshold;
            if (cond.type === 'lp_sentiment_sustained') {
        ...
        const playerSectors = portfolio.filter(c => c.status === 'active').map(c => c.sector);
        ...
        const validDecisions = newDecisions.filter(d => d.deadline >= fund.currentMonth);
        const validSecondaries = newSecondaries.filter(s => s.expiresMonth >= fund.currentMonth);
        const validBuyouts = newBuyouts.filter(b => b.expiresMonth >= fund.currentMonth);
        ...
        const newAchievements = achievementCtx
          ...
          : [];
        const allAchievements = [...(state.unlockedAchievements || []), ...newAchievements];
        ...
        set({
          ...
          fundraisingEvents: [...state.fundraisingEvents, ...newFundraisingEvents],
      ...
      performLPAction: (actionType: LPActionType, params?) => {
        ...
        const effect = calculateLPActionEffect(actionType, state.lpSentiment.score, state.fund, state.portfolio, params);
        if (!effect.canPerform) return { success: false, reason: effect.reason };

        const newScore = clamp(state.lpSentiment.score + effect.sentimentDelta, 0, 100);
        const newLevel = newScore >= 80 ? 'excellent' : newScore >= 60 ? 'good' : newScore >= 40 ? 'neutral' : newScore >= 20 ? 'concerned' : 'critical';

        // Update cooldowns
        const existingCooldowns = (state.fund.lpActionCooldowns || []).filter(c => c.actionType !== actionType);
        const newCooldown = { actionType, availableFromMonth: state.fund.currentMonth + effect.cooldownMonths };
        ...
        if (updatedSnapshots.length > 0) {
          ...
          updatedSnapshots[updatedSnapshots.length - 1] = { ...last, lpScore: newScore };
        }
      ...
      // ============================================================
      resolveBoardMeeting: (meetingId: string, choicesByItemId: Record<string, number>) => {
        const state = get();
        ...
        set({
          portfolio: state.portfolio.map(c => {
            ...
            for (const item of meeting.agendaItems) {
              ...
              if (effects.growthRate) updated.metrics = { ...updated.metrics, growthRate: clamp(updated.metrics.growthRate + effects.growthRate, 0, 1) };
              if (effects.churn) updated.metrics = { ...updated.metrics, churn: clamp(updated.metrics.churn + effects.churn, 0, 1) };
              if (effects.burnRate) updated.metrics = { ...updated.metrics, burnRate: Math.max(0, updated.metrics.burnRate * (1 + effects.burnRate)) };
              if (effects.runway) updated.metrics = { ...updated.metrics, runway: Math.max(0, updated.metrics.runway + effects.runway) };
              if (effects.mrr) updated.metrics = { ...updated.metrics, mrr: Math.max(0, updated.metrics.mrr * (1 + effects.mrr)) };
              if (effects.relationship) updated.relationship = clamp(updated.relationship + effects.relationship, 0, 100);
              if (effects.pmfScore) updated.pmfScore = clamp(updated.pmfScore + effects.pmfScore, 0, 100);
            }
            return updated;
          }),
          boardMeetings: state.boardMeetings.map(m =>
            m.id === meetingId ? { ...m, attended: true, agendaItems: m.agendaItems.map(item => {
              const idx = choicesByItemId[item.id];
              return idx !== undefined ? { ...item, resolved: true, chosenOptionIndex: idx } : item;
            })} : m
          ),
        });
      },
      ...
      invest: (startupId, amount, ownership) => {
        ...
        if (!startup) return { success: false, reason: 'Deal no longer available.' };
        ...
        if (Math.random() * 100 > startup.founderWillingness) {
          return { success: false, reason: `${startup.founderName} declined your investment offer.` };
        }
        ...
        const portfolioCompany: PortfolioCompany = {
          ...
          multiple: amount > 0 ? Math.round((startup.valuation / amount) * 100) / 100 : 0,
          status: 'active',
          origin: 'external',
      ...
      followOn: (companyId, amount) => {
        ...
        const opportunity = state.followOnOpportunities.find(f => f.companyId === companyId);
        ...
        set({
          ...
          portfolio: state.portfolio.map(c =>
            c.id === companyId
              ? { ...c, investedAmount: c.investedAmount + amount, followOnInvested: (c.followOnInvested || 0) + amount, ownership: Math.min(100, newOwnership), influence: getInfluenceLevel(newOwnership) }
              : c
          ),
          followOnOpportunities: state.followOnOpportunities.filter(f => f.companyId !== companyId),
        });
      },
      ...
      skipFollowOn: (companyId) => {
        ...
        set({
          portfolio: state.portfolio.map(c =>
            c.id === companyId
              ? { ...c, ownership: Math.max(1, c.ownership * (1 - opportunity.dilutionIfSkip / 100)) }
              : c
          ),
          followOnOpportunities: state.followOnOpportunities.filter(f => f.companyId !== companyId),
        });
      ...
      sellSecondary: (offerId) => {
        ...
        const cashReceived = company.investedAmount * offer.offerMultiple * (offer.offerPercentage / 100);
        ...
        set({
          ...
          portfolio: state.portfolio.map(c =>
            c.id === offer.companyId
              ? { ...c, ownership: c.ownership - stakeToSell, influence: getInfluenceLevel(c.ownership - stakeToSell) }
              : c
          ),
          secondaryOffers: state.secondaryOffers.filter(s => s.id !== offerId),
        });
      },
      ...
      rejectSecondary: (offerId) => {
        set({ secondaryOffers: get().secondaryOffers.filter(s => s.id !== offerId) });
      },
      ...
      acceptBuyout: (offerId) => {
        ...
        const exitMultiple = company.investedAmount > 0
          ...
          portfolio: state.portfolio.map(c =>
            c.id === offer.companyId
              ? {
                  ...
                  exitData: {
                    acquirerType: offer.buyerType === 'pe' ? 'pe' as const : 'strategic_rival' as const,
      ...
      rejectBuyout: (offerId) => {
        set({ buyoutOffers: get().buyoutOffers.filter(b => b.id !== offerId) });
      },
      ...
      resolveDecision: (decisionId, optionIndex) => {
        ...
        const company = state.portfolio.find(c => c.id === decision.companyId);
        ...
        set({
          portfolio: state.portfolio.map(c => {
            ...
            if (effects.runway) updated.metrics = { ...updated.metrics, runway: Math.max(0, updated.metrics.runway + effects.runway) };
            if (effects.mrr) updated.metrics = { ...updated.metrics, mrr: Math.max(0, updated.metrics.mrr * (1 + effects.mrr)) };
            if (effects.customers) updated.metrics = { ...updated.metrics, customers: Math.max(0, Math.round(updated.metrics.customers * (1 + effects.customers))) };
            if (effects.relationship) updated.relationship = clamp(updated.relationship + effects.relationship, 0, 100);
            if (effects.pmfScore) updated.pmfScore = clamp(updated.pmfScore + effects.pmfScore, 0, 100);
      ...
      hireTalent: (companyId, talentId) => {
        ...
        const probability = calculateHireProbability(candidate, company, state.marketCycle);
        ...
        set({
          portfolio: state.portfolio.map(c => {
            ...
            const updated = { ...c, hiredTalent: [...c.hiredTalent, candidate] };

            if (effects.pmfScore) updated.pmfScore = clamp(updated.pmfScore + (effects.pmfScore as number), 0, 100);
            if (effects.supportScore) updated.supportScore = clamp(updated.supportScore + (effects.supportScore as number), 0, 100);
        ...
        set({
          portfolio: state.portfolio.map(c => {
            ...
            return {
              ...
              relationship: clamp(c.relationship + (action === 'Make Intros' ? 5 : 2), 0, 100),
              pmfScore: clamp(c.pmfScore + (action === 'Give Advice' ? 2 : 0), 0, 100),
            };
          }),
        });
      },
      ...
      launchIncubator: () => {
        ...
        for (let i = 0; i < numCompanies; i++) {
          companies.push({
            startup: generateStartup('pre_seed', state.marketCycle, state.fund.skillLevel + 1),
            mentoringActions: [],
            graduated: false,
          });
        }
        ...
        set({
          fund: { ...state.fund, cashAvailable: state.fund.cashAvailable - cost },
          activeIncubator: {
        ...
        set({
          activeIncubator: {
            ...
            companies: state.activeIncubator.companies.map(c => {
              ...
              return { ...c, mentoringActions: [...c.mentoringActions, action] };
            }),
          },
        });
      },
      ...
      graduateIncubator: () => {
        ...
        const newPortfolio: PortfolioCompany[] = state.activeIncubator.companies.map(ic => ({
          ...ic.startup,
          investedAmount: 0,
          followOnInvested: 0,
          ownership: 2,
        ...
        set({
          ...
          activeIncubator: null,
          incubatorBatches: [...state.incubatorBatches, { ...state.activeIncubator, graduated: true }],
        });
      },
      ...
      assignLabFounder: (projectId, founderId) => {
        ...
        set({
          labProjects: state.labProjects.map(p =>
            p.id === projectId ? { ...p, founder, status: 'assembling' as const } : p
          ),
      ...
      spinOutLab: (projectId) => {
        ...
        const startup = generateStartup('pre_seed', state.marketCycle, state.fund.skillLevel + 2);
        ...
        const investedAmount = Math.round(state.fund.currentSize * randomBetween(0.02, 0.05));
        ...
        const portfolioCompany: PortfolioCompany = {
          ...
          multiple: Math.round((startup.valuation * vMod) / investedAmount * 100) / 100,
          status: 'active',
          origin: 'lab',
          founderState: 'focused',
      ...
      rebirth: () => {
        ...
        set({
          ...
          portfolio: [],
          dealPipeline: [],
          lpSentiment: { ...initialLPSentiment, score: Math.min(70, 50 + rebirthCount * 5) },
          lpReports: [],
          incubatorBatches: [],
          activeIncubator: null,
          labProjects: [],
    ...
    }),
    {
      ...
      merge: (persisted, current) => {
        ...
        if (merged.fund && typeof merged.fund === 'object') {
          ...
          if (f.carryRate === undefined) f.carryRate = 0.20;
          if (f.hurdleRate === undefined) f.hurdleRate = 0.08;
          if (f.gpCommit === undefined) f.gpCommit = Math.round(f.currentSize * 0.01);
