...
export function calculateLPSentiment(state: LPCalculationState): LPSentiment {
  ...
  const marketAdjustment = calculateMarketAdjustment(marketCycle, portfolio, fund);

  // Total
  const rawScore = 50 + portfolioPerformance + eventQuality + valuationMomentum
    + supportQuality + deploymentPace + labQuality + incubatorOutput + marketAdjustment;
...
function calculatePortfolioPerformance(portfolio: PortfolioCompany[]): number {
  ...
  if (weightedMultiple > 2) return clamp(10 + (weightedMultiple - 2) * 5, 10, 20);
  if (weightedMultiple >= 1) return clamp((weightedMultiple - 1) * 10, 0, 10);
  return clamp((weightedMultiple - 1) * 20, -20, 0);
}

function calculateEventQuality(portfolio: PortfolioCompany[], currentMonth: number): number {
  const recentEvents = portfolio.flatMap(c =>
    c.events.filter(e => e.month >= currentMonth - 12 && e.severity === 'severe' && e.sentiment === 'negative')
  );
...
function calculateValuationMomentum(activeCompanies: PortfolioCompany[]): number {
  if (activeCompanies.length === 0) return 0;

  // Use average multiple as proxy for valuation momentum
  const avgMultiple = activeCompanies.reduce((sum, c) => sum + c.multiple, 0) / activeCompanies.length;
...
function calculateSupportQuality(activeCompanies: PortfolioCompany[]): number {
  ...
  const avgSupport = activeCompanies.reduce((sum, c) => sum + c.supportScore, 0) / activeCompanies.length;
...
function calculateDeploymentPace(fund: Fund): number {
  ...
  if (year <= 3) {
    if (deploymentRatio >= 0.20 && deploymentRatio <= 0.60) return 5;
    if (deploymentRatio < 0.10) return -5;
    if (deploymentRatio > 0.70) return -5;
    return 0;
  }
  if (year <= 7) {
    if (deploymentRatio >= 0.50 && deploymentRatio <= 0.80) return 5;
    if (deploymentRatio < 0.30) return -5;
    if (deploymentRatio > 0.90) return -3;
    return 0;
  }
  // Years 8-10
  if (deploymentRatio >= 0.70) return 5;
  if (deploymentRatio < 0.50) return -5;
  return 0;
}

function calculateLabQuality(portfolio: PortfolioCompany[], labProjects: LabProject[]): number {
  ...
  const externalCompanies = portfolio.filter(c => c.origin === 'external' && c.status === 'active');
  const avgLabMultiple = labCompanies.reduce((s, c) => s + c.multiple, 0) / labCompanies.length;
  const avgExternalMultiple = externalCompanies.length > 0
    ? externalCompanies.reduce((s, c) => s + c.multiple, 0) / externalCompanies.length
    : 1.0;
...
function calculateIncubatorOutput(portfolio: PortfolioCompany[], batches: IncubatorBatch[]): number {
  ...
  const totalGrads = graduatedBatches.reduce((s, b) => s + b.companies.filter(c => c.graduated).length, 0);
  const totalCompanies = graduatedBatches.reduce((s, b) => s + b.companies.length, 0);
  const gradRate = totalCompanies > 0 ? totalGrads / totalCompanies : 0;

  const activeIncubators = incubatorCompanies.filter(c => c.status === 'active');
  const avgMultiple = activeIncubators.length > 0
    ? activeIncubators.reduce((s, c) => s + c.multiple, 0) / activeIncubators.length
    : 0;
...
function calculateMarketAdjustment(
  ...
  if (activeCompanies.length > 0) {
    const avgGrowth = activeCompanies.reduce((s, c) => s + c.metrics.growthRate, 0) / activeCompanies.length;
...
export function generateLPPressureReport(state: LPCalculationState): LPPressureReport {
  ...
  if (year <= 3) {
    deploymentRating = deploymentRatio >= 0.20 && deploymentRatio <= 0.60 ? 'strong'
      : deploymentRatio < 0.10 ? 'concerning' : deploymentRatio > 0.80 ? 'critical' : 'adequate';
  } else {
    deploymentRating = deploymentRatio >= 0.50 && deploymentRatio <= 0.85 ? 'strong'
      : deploymentRatio < 0.30 ? 'concerning' : 'adequate';
  }

  // Breakout companies (>3x multiple)
  const breakoutCompanies = activeCompanies.filter(c => c.multiple > 3).length;

  // Red flag count
  const recentSevereEvents = portfolio.flatMap(c =>
    c.events.filter(e => e.month >= fund.currentMonth - 12 && e.severity === 'severe' && e.sentiment === 'negative')
  ).length;
  const failedThisYear = portfolio.filter(c =>
    c.status === 'failed' && c.events.some(e => e.month >= fund.currentMonth - 12)
  ).length;
  const redFlagCount = recentSevereEvents + failedThisYear;

  // Reserves rating
  const remainingCashRatio = fund.cashAvailable / fund.currentSize;
  const reservesRating = remainingCashRatio > 0.25 ? 'strong'
    : remainingCashRatio > 0.10 ? 'adequate'
    : remainingCashRatio > 0.05 ? 'concerning' : 'critical';
  ...
  const hasStudio = studioCount > 0 || labProjects.length > 0 || incubatorBatches.length > 0;
  ...
  if (!hasStudio) {
    ...
    const avgStudioMultiple = [...labCompanies, ...incubCompanies]
      .reduce((s, c) => s + c.multiple, 0) / studioCount;
    studioROI = avgStudioMultiple > 2 ? 'strong'
      : avgStudioMultiple > 1 ? 'adequate' : 'concerning';
  }

  // Overall grade
  const ratings = [deploymentRating, reservesRating, studioROI !== 'N/A' ? studioROI : 'adequate'];
  const ratingScore = (r: string) => r === 'strong' ? 4 : r === 'adequate' ? 3 : r === 'concerning' ? 2 : 1;
  let gradeScore = ratings.reduce((s, r) => s + ratingScore(r), 0) / ratings.length;
  ...
  const overallGrade = gradeScore >= 3.5 ? 'A' : gradeScore >= 2.8 ? 'B' : gradeScore >= 2.0 ? 'C' : gradeScore >= 1.5 ? 'D' : 'F';
...
export function generateLPReport(state: LPCalculationState): LPReport {
  ...
  const year = getGameYear(fund.currentMonth);
  const activeCompanies = portfolio.filter(c => c.status === 'active');
  const exitedThisYear = portfolio.filter(c =>
    c.status === 'exited' && c.exitData && c.exitData.month >= fund.currentMonth - 12
  );
  const failedThisYear = portfolio.filter(c =>
    c.status === 'failed' && c.events.some(e => e.month >= fund.currentMonth - 12)
  );
  ...
  const topPerformers = [...activeCompanies]
    ...
    .slice(0, 3)
    .map(c => `${c.name} (${c.multiple.toFixed(1)}x, ${formatCurrency(c.currentValuation)} valuation)`);

  // Exits
  const exits = exitedThisYear.map(c =>
  ...
  // Write-offs
  const writeOffs = failedThisYear.map(c =>
  ...
  if (exitedThisYear.length > 0) {
    const totalReturned = exitedThisYear.reduce((s, c) => s + (c.exitData?.exitValue || 0), 0);
    highlights.push(`${exitedThisYear.length} successful exit${exitedThisYear.length > 1 ? 's' : ''} returning ${formatCurrency(totalReturned)}`);
  }
  if (topPerformers.length > 0 && activeCompanies[0]?.multiple > 2) {
    highlights.push(`${activeCompanies.filter(c => c.multiple > 2).length} portfolio companies above 2x multiple`);
  }
  ...
  if (strugglingCompanies.length > 0) {
    concerns.push(`${strugglingCompanies.length} portfolio companies below 0.5x multiple`);
  }
  const burnoutFounders = activeCompanies.filter(c => c.founderState === 'burned_out' || c.founderState === 'defensive');
  if (burnoutFounders.length > 0) {
    concerns.push(`${burnoutFounders.length} companies with founder challenges requiring close attention`);
  }
  const lowRunway = activeCompanies.filter(c => c.metrics.runway < 6);
  if (lowRunway.length > 0) {
    concerns.push(`${lowRunway.length} companies with less than 6 months runway`);
  }
  if (failedThisYear.length > 0) {
    concerns.push(`${failedThisYear.length} write-off${failedThisYear.length > 1 ? 's' : ''} this year`);
  }
  ...
  const netTvpi = fund.currentSize > 0
    ? Math.round(((fund.tvpiEstimate * fund.currentSize - fund.totalFeesCharged - fund.carryAccrued) / fund.currentSize) * 100) / 100
    : grossTvpi;
  const grossIrr = fund.irrEstimate;
  const yearsElapsed = fund.currentMonth / 12;
  const netIrr = yearsElapsed > 0 && netTvpi > 0
    ? Math.round((Math.pow(Math.max(0.01, netTvpi), 1 / yearsElapsed) - 1) * 100 * 100) / 100
    : 0;
...
export function getLPEffects(sentiment: LPSentiment): {
  ...
  const commitmentMod = 0.70 + (score / 100) * 0.60; // 0.7 to 1.3

  // dealflowMod: ±20% on deal quality
  const dealflowMod = 0.80 + (score / 100) * 0.40; // 0.8 to 1.2
  ...
  const coInvestorMod = 0.75 + (score / 100) * 0.50; // 0.75 to 1.25
...
export function calculateLPActionEffect(
  ...
  if (existingCooldown && existingCooldown.availableFromMonth > fund.currentMonth) {
    const monthsLeft = existingCooldown.availableFromMonth - fund.currentMonth;
    return { sentimentDelta: 0, cashCost: 0, cooldownMonths: 0, canPerform: false, reason: `Available in ${monthsLeft} month${monthsLeft > 1 ? 's' : ''}.` };
  }
  ...
  switch (actionType) {
    ...
    case 'lp_day': {
      if (currentSentiment < 30) {
        return { sentimentDelta: 0, cashCost: 0, cooldownMonths: 0, canPerform: false, reason: 'LP sentiment too low (need ≥ 30) to hold LP Day.' };
      }
      const cost = Math.round(fund.currentSize * randomBetween(0.001, 0.005));
      if (cost > fund.cashAvailable) {
        return { sentimentDelta: 0, cashCost: 0, cooldownMonths: 0, canPerform: false, reason: 'Insufficient cash for LP Day.' };
      }
      return {
    ...
    case 'coinvest_opportunity': {
      const eligibleCompany = portfolio.find(c =>
        c.status === 'active' && c.multiple > 1.5 &&
        (params?.companyId ? c.id === params.companyId : true)
      );
      if (!eligibleCompany) {
        return { sentimentDelta: 0, cashCost: 0, cooldownMonths: 0, canPerform: false, reason: 'No active company with multiple > 1.5x available.' };
      }
      return {
    ...
    case 'early_distribution': {
      const year = Math.floor(fund.currentMonth / 12) + 1;
      if (year < 3) {
        return { sentimentDelta: 0, cashCost: 0, cooldownMonths: 0, canPerform: false, reason: 'Early distributions not available until Year 3.' };
      }
      const amount = params?.amount ?? 0;
      if (amount <= 0) {
        return { sentimentDelta: 0, cashCost: 0, cooldownMonths: 0, canPerform: false, reason: 'Specify a distribution amount.' };
      }
      if (amount > fund.cashAvailable) {
        return { sentimentDelta: 0, cashCost: 0, cooldownMonths: 0, canPerform: false, reason: 'Insufficient cash for distribution.' };
      }
