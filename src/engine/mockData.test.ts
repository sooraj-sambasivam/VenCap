...
describe("SECTOR_DATA", () => {
  ...
  it("each sector has nameWords and descriptionTemplates", () => {
    for (const [name, data] of Object.entries(SECTOR_DATA)) {
      ...
      expect(
        data.descriptionTemplates.length,
...
describe("generateCoInvestor", () => {
  it("returns a valid co-investor", () => {
    ...
    expect(["tier1", "friendly", "competitive", "strategic"]).toContain(
      ci.tier,
    );
...
describe("generateStartup", () => {
  ...
  it("market data is present", () => {
    ...
    expect(["low", "medium", "high", "saturated"]).toContain(
      s.marketData.competitionLevel,
    );
  });
  ...
  it("higher skill level produces better startups on average", () => {
    let lowSkillAvgGrowth = 0,
      highSkillAvgGrowth = 0;
    const n = 200;
    for (let i = 0; i < n; i++) {
      lowSkillAvgGrowth += generateStartup("seed", "normal", 1).metrics
        .growthRate;
      highSkillAvgGrowth += generateStartup("seed", "normal", 5).metrics
        .growthRate;
    }
    lowSkillAvgGrowth /= n;
